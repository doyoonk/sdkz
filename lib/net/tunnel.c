...
LOG_MODULE_DECLARE(net, LOG_LEVEL_DBG);
...
#if defined(CONFIG_NET_COMMON_TUNNEL_PEER_ADDR)
#define NET_COMMON_TUNNEL_PEER_ADDR CONFIG_NET_COMMON_TUNNEL_PEER_ADDR
#else
#define NET_COMMON_TUNNEL_PEER_ADDR ""
#endif

#if defined(CONFIG_NET_COMMON_TUNNEL_MY_ADDR)
#define NET_COMMON_TUNNEL_MY_ADDR CONFIG_NET_COMMON_TUNNEL_MY_ADDR
#else
#define NET_COMMON_TUNNEL_MY_ADDR ""
#endif
...
int init_tunnel(void)
{
	...
	if (NET_COMMON_TUNNEL_PEER_ADDR[0] == '\0') {
		LOG_INF("Tunnel peer address not set.");
		return 0;
	}

	if (!net_ipaddr_parse(NET_COMMON_TUNNEL_PEER_ADDR,
			      strlen(NET_COMMON_TUNNEL_PEER_ADDR),
			      &peer)) {
		LOG_ERR("Tunnel peer address \"%s\" invalid.",
			NET_COMMON_TUNNEL_PEER_ADDR);
		return -EINVAL;
	}
